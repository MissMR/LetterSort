...
public class AZSideBarView extends View {
    ...
    public void setmLetters(List<String> mLetters) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        float contentLeft = 0;
        float contentRight = mBarWidth + mBarPadding;
        float viewHeight = (float) mTextSize * 2.2F * mLetters.size();
        float contentTop = (getMeasuredHeight() - viewHeight) / 2 > 0 ? (getMeasuredHeight() - viewHeight) / 2 : 0;
        float contentBottom = viewHeight + (getMeasuredHeight() - viewHeight) / 2 < getMeasuredHeight() ? viewHeight + (getMeasuredHeight() - viewHeight) / 2 : getMeasuredHeight();
        mSlideBarRect.set(contentLeft, contentTop, contentRight, contentBottom);
        setMeasuredDimension((int) contentRight, (int) viewHeight);
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        mPreSelect = mSelect;
        mNewSelect = (int) ((y - mSlideBarRect.top) / (mSlideBarRect.bottom - mSlideBarRect.top) * mLetters.size());
